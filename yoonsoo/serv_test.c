...
void* control_motor(void* arg) { //윤수가 해야할 거, 이거에 전체적으로 전역변수처럼 선언해서, warning 0일때는 정상작동, 아니면 따로 조치하는 식으로 작성
		...
		if (clnt_count < CLNT_SIZE) {
			...
			*/
			...
			if (strncmp(buffer, "CONTROL", strlen("CONTROL")) == 0) {
				...
				*/
			// 윤수가 해야할거, 여기도 스레드 생성해서 어떤 식으로 제어할 지 실행 코드 작성이 필요함
